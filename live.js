(() => {
  ...
  function renderPayload(payload) {
    window.liveLeaderboardJson = payload;
    output.classList.remove('error');
    output.textContent = JSON.stringify(payload, null, 2);
    status.textContent = `Updated ${new Date(payload.generatedAt).toLocaleTimeString()}`;
  }
  ...
  async function refresh() {
    try {
      ...
      if (csvText === lastCSV) {
        status.textContent = `No change ${new Date().toLocaleTimeString()}`;
        return;
      }
      lastCSV = csvText;
      renderPayload(buildPayload(csvText));
    } catch (err) {
